...
Write-Host "`n[1/3] Running tests (Aurum.Core.SelfTests)..." -ForegroundColor Yellow
...
Write-Host "`n[2/3] Preparing output directory: $OutputDir" -ForegroundColor Yellow
if (Test-Path $OutputDir) {
    Remove-Item -Path $OutputDir -Recurse -Force
}
New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null

Write-Host "`n[3/3] Publishing standalone single-file binary..." -ForegroundColor Yellow
