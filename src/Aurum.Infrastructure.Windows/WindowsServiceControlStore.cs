...
public sealed class WindowsServiceControlStore : IServiceControlStore
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // Windows only honours the delayed flag for Automatic services, and it lives in a
        // separate configuration level than the start type.
        if (delayedAutoStart is bool delayed && startMode == ServiceStartMode.Automatic)
        {
            WriteDelayedAutoStart(service, serviceName, delayed);
        }

        return Task.CompletedTask;
    }

    private static void WriteDelayedAutoStart(SafeServiceHandle service, string serviceName, bool delayed)
    {
        var buffer = Marshal.AllocHGlobal(sizeof(int));
        try
        {
            Marshal.WriteInt32(buffer, delayed ? 1 : 0);
            if (!ServiceControlNativeMethods.ChangeServiceConfig2(service, SERVICE_CONFIG_DELAYED_AUTO_START_INFO, buffer))
            {
                throw new Win32Exception(
                    Marshal.GetLastWin32Error(),
                    $"Не удалось восстановить отложенный автозапуск службы '{serviceName}'.");
            }
        }
        finally
        {
